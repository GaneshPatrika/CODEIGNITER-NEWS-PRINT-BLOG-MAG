...
class Post_model extends CI_Model
{
	public function set_filter_query()
	{
		...
		$this->db->select('posts.* , users.username as username, users.slug as user_slug,users.avatar as profile_pic, categories.name as category_name, categories.slug as category_slug, categories.parent_id as category_parent_id, (SELECT slug FROM categories WHERE id = category_parent_id) as parent_category_slug');
	...
	public function get_posts()
	{
		...
		//get all posts
	public function get_posts_limit($per_page, $offset)
	{
		$this->set_filter_query();
		$this->db->order_by('posts.created_at', 'DESC');
	...
	public function get_popular_posts($limit)
	{
		...
		//get popular posts
	public function get_popular_posts_limit($per_page, $offset)
	{
		$this->set_filter_query();
		$this->db->order_by('hit', 'DESC');
		$this->db->limit($per_page, $offset);
	...
	public function get_our_picks($limit)
	{
		...
		//get picked posts
	public function get_our_picks_articles($limit)
	{
		$this->set_filter_query();
		$this->db->where('is_picked', 1);
	...
	//get picked posts
	public function get_our_picks_video($limit)
	{
		$this->set_filter_query();
		$this->db->where('is_picked', 1);
