...
class Navigation_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT * FROM ((SELECT categories.id AS item_id, categories.lang_id AS item_lang_id, categories.name AS item_name,categories.iconcat AS icon_cat, categories.slug AS item_slug, categories.category_order AS item_order, 'header' AS item_location, 'category' AS item_type, '#' AS item_link, categories.parent_id AS item_parent_id, categories.show_on_menu AS item_visibility FROM categories) UNION
		(SELECT pages.id AS item_id, pages.lang_id AS item_lang_id, pages.title AS item_name,pages.icontag AS icon_cat, pages.slug AS item_slug, pages.page_order AS item_order, pages.location AS item_location, 'page' AS item_type, pages.link AS item_link, pages.parent_id AS item_parent_id, pages.page_active AS item_visibility FROM pages)) AS menu_items ORDER BY item_order");
	...
	{
		$lang_id = clean_number($lang_id);
		$query = $this->db->query("SELECT * FROM ((SELECT categories.id AS item_id, categories.name AS item_name,categories.iconcat AS icon_cat, categories.slug AS item_slug, categories.category_order AS item_order, 'header' AS item_location, 'category' AS item_type, '#' AS item_link FROM categories WHERE categories.show_on_menu = 1 AND categories.parent_id = 0 AND categories.lang_id=" . $lang_id . ") UNION
		(SELECT pages.id AS item_id, pages.title AS item_name,pages.icontag AS icon_cat, pages.slug AS item_slug, pages.page_order AS item_order, pages.location AS item_location, 'page' AS item_type, pages.link AS item_link FROM pages WHERE pages.page_active = 1 AND pages.parent_id = 0 AND pages.lang_id=" . $lang_id . ")) AS menu_items ORDER BY item_order");
		if (!empty($query->result())) {
			return $query->result();
		}
		return array();
	...
	public function get_sub_links($parent_id, $type)
	{
		...
		if ($type == 'page') {
			$this->db->select("pages.title as item_name,pages.icontag as iconcat, pages.slug as item_slug, pages.link as item_link, 'page' as item_type");
			...
			$this->db->select("categories.name as item_name,categories.iconcat as iconcat, categories.slug as item_slug, 'category' as item_type, categories.parent_id as item_parent_id, (SELECT slug FROM categories WHERE id = item_parent_id) as item_parent_slug");
